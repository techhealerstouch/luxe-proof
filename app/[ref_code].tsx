...
export default function NfcPublicProfile() {
  ...
  // Get auth token from localStorage - optional for public view
  ...
  // Helper function to extract ref_code from URL
  const extractRefCodeFromUrl = (url: string): string | null => {
    try {
      const urlObj = new URL(url);
      const pathParts = urlObj.pathname.split("/").filter(Boolean);
      // Return the last part of the path as ref_code
  ...
  // Helper function to validate ref_code format (adjust based on your format)
  const isValidRefCodeFormat = (code: string): boolean => {
    // Adjust this regex based on your ref_code format
    // Example: 6-12 alphanumeric characters
    return /^[A-Z0-9]{6,12}$/i.test(code);
  };

  // Initialize ref_code from URL parameter on component mount
  useEffect(() => {
    if (params?.refCode) {
      const urlRefCode = Array.isArray(params.refCode)
        ? params.refCode[0]
        : params.refCode;
      setRefCode(urlRefCode.toUpperCase());
      // Auto-load profile if URL contains ref_code
      viewPublicProfile(urlRefCode);
    }
  }, [params?.refCode]);
  ...
  // Handle scanned QR code data (URL or direct ref_code)
  const handleScannedData = (scannedData: string) => {
    let extractedRefCode = scannedData.toUpperCase();

    // Check if scanned data is a URL
    if (scannedData.startsWith("http")) {
      const extracted = extractRefCodeFromUrl(scannedData);
      if (extracted) {
        extractedRefCode = extracted.toUpperCase();
      } else {
        setMessage({
          type: "error",
          text: "Could not extract reference code from URL",
        });
        clearMessage();
        return;
      }
    ...
    if (!isValidRefCodeFormat(extractedRefCode)) {
      setMessage({
        type: "error",
        text: "Invalid reference code format",
      });
      clearMessage();
      return;
    }
    ...
    setMessage({
      type: "success",
      text: `Reference code scanned successfully: ${extractedRefCode}`,
    });
    clearMessage();

    // Auto-load profile
    viewPublicProfile(extractedRefCode);
  };
  ...
  // Handle input field changes with URL parsing
  const handleRefCodeInput = (value: string) => {
    let processedValue = value.trim();

    // Check if input is a URL
    if (processedValue.startsWith("http")) {
      const extracted = extractRefCodeFromUrl(processedValue);
      if (extracted) {
        processedValue = extracted;
        setMessage({
          type: "info",
          text: `Extracted reference code from URL: ${extracted}`,
        });
        clearMessage();
      }
    }

    setRefCode(processedValue.toUpperCase());
  };
  ...
  const viewPublicProfile = async (code?: string) => {
    ...
    if (!isValidRefCodeFormat(codeToUse)) {
      setMessage({
        type: "error",
        text: "Invalid reference code format. Please check and try again.",
      });
      clearMessage();
      return;
    }
    ...
    try {
      ...
      if (result.valid && result.data) {
        ...
        setMessage({
          type: "success",
          text: "Product profile loaded successfully",
        });

        // Update URL without page reload if not already in URL
        if (!params?.refCode || params.refCode !== codeToUse) {
          router.push(`/${codeToUse}`, { scroll: false });
        }
      } else {
      ...
      setMessage({ type: "error", text: "Failed to load product profile" });
      setPublicProfile(null);
    } finally {
